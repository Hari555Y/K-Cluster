...
def generate_random_points(D, n1, alpha, groups_underreport_percent):
    ...
    for i in range(D):
        for j in range(D):
            ...
            curgroup = 2
            if group<groups_underreport_percent/100:
                curgroup = 1
            if curgroup == 2:
                points_in_current_group = n1
...
if __name__ == "__main__":
    ...
    percund = int(input("Percent of groups underreported: "))
  #  k = int(input("Enter the number of clusters: ")) 
   # k is now taken as a function of the number of points in the dataset
    ...
    for alpha in range(10,90,10):
        k1 = max(1 ,int((D*D*(n1+n1))/100))
        k2 = max(1, int((D*D*(n1  + int((alpha*n1)/(100-alpha)) + n1))/100))
        points , actual_points = generate_random_points(D, n1 , alpha, percund)
    # actual_points = generate_random_points(D, n1  + int((alpha*n1)/100), n2)
        ...
        # plt.show()
    titl = f"Graph for k= {k1} and {percund}% blocks having underreporting"
    plt.plot(range(10,90,10), cost)
    plt.xlabel("Percentage of Density Underreported")
    plt.ylabel("Log Squared Distance Sum")
    plt.title(titl)
    plt.show()